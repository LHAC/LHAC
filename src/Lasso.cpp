...
Lasso::Lasso(const char *filename, bool isCached)
{
    _isCached = isCached;
    ...
    _aTb = new double[_p];
    lcdgemv(CblasColMajor, CblasTrans, Dset->X, Dset->y, _aTb, (int)_N, (int)_p, (int)_p);
...
double Lasso::computeObject(double* wnew)
{
    ...
    if (_isCached) {
        ...
        order2 = lcddot((int)_p, _Ax, 1, wnew, 1);
